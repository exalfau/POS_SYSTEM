...
{
    public partial class frmUserDetailView : SampleDetailView
    {
        public string name;
        public bool custom, today, week, month, year, fiveYear, allData;

        public frmUserDetailView()
        {
            InitializeComponent();
        }

        private void frmUserDetailView_Load(object sender, EventArgs e)
        {
        ...
        private void LoadDataWithDateRange(string datePart, int range)
        {
            ...
            string qry = @"SELECT sd.id, t.invoice_id, t.date,  s.name, c.name,  p.name, sd.quantity, sd.price, sd.total_price, t.information
                            ...
                            WHERE a.username = '" + name + "' AND (c.name LIKE '%" + txtSearch.Text + "%' OR p.name LIKE '%" + txtSearch.Text + "%' " +
